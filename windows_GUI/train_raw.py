...
if __name__ == '__main__':
    ...
    #训练模型，设置epochs为400、验证频率为1
    #model.fit(dataset_cat, 
    ...
    def train():
        for epoch in range(epochs):
            ...
            for images, labels in dataset_cat:
                ...
                step += 1
                #print(loss)
                tf.print('epoch:',epoch,'step:',step)
                break
    train()
    
    #_, keras_file = tempfile.mkstemp('.h5')
    #tf.keras.models.save_model(student_net, keras_file, include_optimizer=False)
    student_net.save('student_net.h5')
   
    #-------------Define model for pruning.-----------------
    prune_low_magnitude = tfmot.sparsity.keras.prune_low_magnitude
    pruning_params = {
        'pruning_schedule': tfmot.sparsity.keras.PolynomialDecay(initial_sparsity=0.50,final_sparsity=0.8,begin_step=50,end_step=350)
    }
